...
def create_app():
    ...
    def talkToGroq():
        ...
        # Format cleanly — only expose your metadata fields to the LLM
        # This prevents the LLM from picking up pgvector's internal UUIDs
        relevant = ", ".join(
        ...
        design_prompt = (
            ...
            f"You MUST end your response with exactly this format on the last line - "
            f"no extra text, no 'id=' prefix, just the raw IDs in brackets: "
            f"[id1], [id2], [id3]"
        )
        ...
        # Return key must be "content" — that's what the frontend reads
